...
from .profiler import KuzuQueryProfiler
...
class MemoryADD:
        ...
        for attempt in range(retries):
            try:
                _ = self.memory.add(
                    ...
                    user_id=user_id, 
                    metadata=metadata,
                    prompt=self.custom_instructions
                )
                return
    ...
    def process_all_conversations(self, max_workers=10):
        ...
        logger.info(f"Processing {len(self.data)} conversations with max_workers={max_workers}")
        ...
        kuzu_profiler = KuzuQueryProfiler(self.memory.graph)
        stats = kuzu_profiler.get_stats()
        logger.info(f"Kuzu Graph Stats: {stats}")